...
# パラメータ設定
basedir = 'C:/Projects/Gurobi/createRoute/moriya/'
resultdir = 'result/'
debugdir = 'debug/'
nodefile = "nodes.csv"
edgefile = "edges.csv"
...
minLength = 5000  # 最小路線長 それ以下の路線は作られない
span = 2500  # 路線候補を作る間隔
...
routeFileName = "routeList" + now.strftime("%Y%m%d%H%M%S") + ".csv"
routeCombiFileName = "routeCombination" + now.strftime("%Y%m%d%H%M%S") + ".csv"
...
def load_data(nodeData, edgeData, distanceMatrixData, flowData=None):
    ...
    if flowData is not None and os.path.exists(flowData):
...
def createTerminalCombinationNoRoop(hubs):
    list = []
    for hub1 in hubs:
        for hub2 in hubs:
            if list not in [hub1, hub2]:
...
def createRouteList(N, E, D, F, G, listTerminal, minLength, maxLength, span):
        ...
        if hub[0] == hub[1]:
            for length in range(minLength, maxLength + span, span):
                ...
                if selectN is not None:
                    routes_condition[count] = str(hub[0]) + "_" + str(hub[1]) + "_" + str(length) + "_" + str("R")
                    ...
                    count += 1
                    # TODO 都度出力
                    outputRouteList(routes_condition, routes_node, routes_edge, routes_length, routes_objVal, routes_CalcTime)
            ...
            # hub間路線網パターン作成
            bMaxWeight = True  # ルートを重み最大化で計算するか．　しないなら路線長最小化．
            ...
            if bMaxWeight:
                for length in range(minLength, maxLength + span, span):
                    ...
                    if shortestLength < length:
                        ...
                        if selectE is not None:
                            routes_condition[count] = str(hub[0]) + "_" + str(hub[1]) + "_" + str(length) + "_" + str("F")
                ...
                selectE_r, routeLength_r, model_r = createRoute_maxWeight(N_selected, E, hub[0], hub[1], length)
                ...
                # 経路長最小化で計算
                maxBeta = 100  # 重み何倍まで計算するか
                betaSpan = 10  # 間隔
                for beta in range(10, maxBeta + betaSpan, betaSpan):
                    beta = beta / 10
                    print("solve:" + str(hub[0]) + ", " + str(hub[1]) + "," + str(beta))
                    # 最大路線長を超えないパターンを作成

                    # selectE, routeLength, model = createRoute(N, E, hub[0], hub[1], length, model)
                    selectE, routeLength, model = createRoute_minDist(N, E, hub[0], hub[1], beta * weightShortestPath)

                    if selectE is not None:
                        routes_condition[count] = str(hub[0]) + "_" + str(hub[1]) + "_" + str(beta) + "_" + str("F")
                        selectN, weight = edgeConvertnodeswithWeight(selectE, N)
                        routes_node[count] = selectN
                        routes_edge[count] = selectE
                        routes_length[count] = routeLength
                        routes_objVal[count] = weight  # 重みを代入
                        routes_CalcTime[count] = model.Runtime
                        count += 1

                    # すでに上記路線が選ばれた場合の路線を作成する
                    N_selected = updateNodeWeight(N, selectN)
                    selectE_r, routeLength_r, model_r = createRoute_minDist(N_selected, E, hub[0], hub[1], beta)
                if selectE_r is not None:
                    selectN_r, weight_r = edgeConvertnodeswithWeight(selectE_r, N_selected)
                routes_condition[count] = str(hub[0]) + "_" + str(hub[1]) + "_" + str(beta) + "_" + str("R")
                routes_node[count] = selectN_r
                routes_edge[count] = selectE_r
                routes_length[count] = routeLength_r
                routes_objVal[count] = weight_r
                routes_CalcTime[count] = model_r.Runtime
                count += 1
...
def createRoute_minDist(nodes, edges, s, t, minWeight):
    ...
    for v in nodes:
        for e in edges:
            if v == e[0]:
                if v == s:
                    model_rmf.addConstr(quicksum(x[i, j] for (i, j) in edges if i == v) - quicksum(x[j, i] for (j, i) in edges if i == v) == 1)
                elif v == t:
                    model_rmf.addConstr(quicksum(x[i, j] for (i, j) in edges if i == v) - quicksum(x[j, i] for (j, i) in edges if i == v) == -1)
                else:
                    model_rmf.addConstr(quicksum(x[i, j] for (i, j) in edges if i == v) - quicksum(x[j, i] for (j, i) in edges if i == v) == 0)
...
def createRoute_maxWeight(nodes, edges, s, t, dist, F=None):
    ...
    if F == None:
        ...
        model_rmf.setObjective(quicksum(y[i] * y[j] * F[(i, j)] for i in nodes for i in nodes if i != j), GRB.MAXIMIZE)
    ...
    if bInit:
        ...
        for v in N:
            for e in E:
                if v == e[0]:
                    if v == s:
                        model_rmf.addConstr(quicksum(x[i, j] for (i, j) in E if i == v) - quicksum(x[j, i] for (j, i) in E if i == v) == 1)
                    elif v == t:
                        model_rmf.addConstr(quicksum(x[i, j] for (i, j) in E if i == v) - quicksum(x[j, i] for (j, i) in E if i == v) == -1)
                    else:
                        model_rmf.addConstr(quicksum(x[i, j] for (i, j) in E if i == v) - quicksum(x[j, i] for (j, i) in E if i == v) == 0)
        """
        for (i, j) in E:
        ...
        model_rmf.setObjective((quicksum(x[i, j] * (N[i][2] + N[j][2]) for (i, j) in E) + (N[s][2] + N[t][2])) / 2, GRB.MAXIMIZE)
...
def createTSPModel(N, E, s, dist, model):
    ...
    for v in N:
        for e in E:
            if v == e[0]:
                # 始点から終点に流れる
                model_rTSP.addConstr(quicksum(x[i, j] for (i, j) in E if i == v) - quicksum(x[j, i] for (j, i) in E if i == v) == 0)
...
def createRoute_roop(N, D, s, maxdist):
    ...
    model_roop.addConstr(quicksum(x[i, j] for i in nodes for j in nodes if i != j) == quicksum(y[i] for i in nodes), "numNode&Link")
...
def calcBestRouteCombination(N, E, routes_condition, routes_node, routes_edge, routes_length, total_length, mainNode):
    ...
    print("case:" + str(total_length) + " hubs:" + str(selectHubs) + " node:" + str(selectNode) + "_route:" + str(selectRoute) + "_length:" + str(length))
...
def calcRouteCombination_Tree(N, E, routes_node, routes_edge, routes_length, total_length):
    ...
    print("case " + str(total_length) + ": node:" + str(selectNode) + "_route:" + str(selectRoute) + "_length:" + str(length))
...
def outputRouteList(routes_condition, routes_node, routes_edge, routes_length, routes_objVal, routes_CalcTime):
    ...
    for key, condition in routes_condition.items():
        series = pd.Series(
            [condition, routes_node[key], routes_edge[key], routes_length[key], routes_objVal[key], routes_CalcTime[key]],
...
def outputRouteCombination(totalLength, routes_id, routes_hubs, routes_node, routes_length, routes_objVal, routes_CalcTime):
    outRouteList = pd.DataFrame(index=[],
                                columns=["totalLength",
                                         "routeID",
                                         "hubs",
    ...
    for key, routeId in routes_id.items():
        series = pd.Series(
            [totalLength[key], routeId, routes_hubs[key], routes_node[key], routes_length[key], routes_objVal[key], routes_CalcTime[key]],
            index=outRouteList.columns)
        outRouteList = outRouteList.append(series,
                                           ignore_index=True)
...
def vrp(demand, c, Q, beta):
    ...
    model.addConstr(quicksum(c[demand[i], demand[j]] * x[i, j] for i in V for j in V if j > i) <= beta * m)
...
def solveVRP_numVehicles(nodes, D, route_nodes, pattern, depot, tripUnit, distTripMin, hour, Q, dLimit, busRouteLength, ratio):
    print("---------------------------↓create_demand_pattern↓-------------------------")
    min = 99
    max = 0
    demand = pd.DataFrame(index=[], columns=["busRouteLength", "ratio", "pid", "from", "to"])
    demand = createDRTDemand(nodes, D, pattern, hour, tripUnit * ratio, distTripMin, route_nodes, demand, busRouteLength, ratio)
    ...
    print("pattans:" + str(pattern) + " min:" + str(min_customer) + " max:" + str(max_customer) + " ave:" + str(ave_customer))
    print("---------------------------↓solve_VRP↓-------------------------")
    count = 0
    ...
    for i in range(pattern):
        ...
        vehicles.append(m.X)
        lengths.append(model_vrp.objVal)
    ...
    print("length:" + str(lengths))
    print("required vehicles=" + str(vehicles))
    ave_v = sum(vehicles) / len(vehicles)
    for v in vehicles:
        if min > v and v > 0:
            min = v
        if max < v:
...
if __name__ == "__main__":
    ...
    # 環状線なしパターン
    # listTerminal = createTerminalCombinationNoRoop(hubs)
    # 路線パターン作成
    routes_condition, routes_node, routes_edge, routes_length, routes_objVal = createRouteList(N, E, D, F, G, listTerminal, minLength, maxLength, span)
    ...
    for length in range(minTotalLength, maxTotalLength + totalLengthSpan, totalLengthSpan):
        nodes, id, routeLength, model_b = calcBestRouteCombination(N, E, routes_node, routes_edge, routes_length, length)
        if id is not None:
            totalLengthList[count] = length
            routes_id_b[count] = id
            routes_node_b[count] = nodes
            ...
            # TODO 都度出力
            outputRouteCombination(totalLengthList, routes_id_b, routes_node_b, routes_length_b, routes_objVal_b, routes_CalcTime_b)

    routeCombiFileName = "routeCombination" + now.strftime(
        "%Y%m%d%H%M%S") + ".csv"
    """

    mainNode = None
    for length in range(minTotalLength, maxTotalLength + totalLengthSpan, totalLengthSpan):
        selectNodes, id, routeLength, model_b, selectHubs = calcBestRouteCombination(N, E, routes_condition, routes_node, routes_edge, routes_length, length, mainNode)
        if mainNode is None:
            mainNode = selectHubs
        ...
        if id is not None:
            ...
            # TODO 都度出力
            outputRouteCombination(totalLengthList, routes_id_b, routes_hubs_b, routes_node_b, routes_length_b, routes_objVal_b, routes_CalcTime_b)
    ...
    for ratio in range(minRatio, maxRatio + ratioSpan, ratioSpan):
        # バス路線無しver
        demand, numCustomers, lengths, vehicles = solveVRP_numVehicles(N, D, None, K, mainhub, tripUnit, distTripMin, hour, Q, dLimit, 0, ratio)
        ...
        for s in routes_node_b:
            demand, numCustomers, lengths, vehicles = solveVRP_numVehicles(N, D, routes_node_b[s], K, mainhub, tripUnit, distTripMin, hour, Q, dLimit, totalLengthList[s], ratio)
